...
class progressive(ic.instance):

    def __init__(self, keyword):

        super().__init__(keyword)

    def create_instance(self, folder = 'instances/progressive'):

        # Create progressive instances
        try:
                ...
                if period1 < period2:
                    ...
                    for location in self.locations:
                        ...
                        for customer in self.captured_customers[location]:
                            ...
                            self.coefficients[period1][period2][location][customer] -= self.penalties[customer] * sum(sum(self.spawning[period4][customer] for period4 in self.periods if period4 > period1 and period4 <= period3) for period3 in self.periods if period3 > period1 and period3 < period2)