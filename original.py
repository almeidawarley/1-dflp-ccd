...
class original(fm.formulation):
    ...
    def create_c2(self):
        ...
        self.mip.addConstrs((self.var['b']['0', customer] == self.ins.starts[customer] for customer in self.ins.customers), name = 'c2')

    def create_c3(self):
        # Create constraint 3

        self.mip.addConstrs((self.var['c'][period, customer] == (1 + self.ins.alphas[customer]) * self.var['b'][self.ins.previous_period(period), customer] + self.ins.betas[customer] for period in self.ins.periods for customer in self.ins.customers), name = 'c3')
